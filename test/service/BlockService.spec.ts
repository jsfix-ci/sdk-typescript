...
describe('BlockService', () => {
    ...
    it('should validate transaction', async () => {
        const result = await lastValueFrom(blockService.validateTransactionInBlock(leaf, UInt64.fromUint(1)));
        expect(result).to.be.true;
    });

    it('should validate transaction - wrong hash', async () => {
        const result = await lastValueFrom(blockService.validateTransactionInBlock(leaf, UInt64.fromUint(2)));
        expect(result).to.be.false;
    });

    it('should validate transaction - emtpy path item', async () => {
        const result = await lastValueFrom(blockService.validateTransactionInBlock(leaf, UInt64.fromUint(4)));
        expect(result).to.be.false;
    });

    it('should validate transaction - error', async () => {
        const result = await lastValueFrom(blockService.validateTransactionInBlock(leaf, UInt64.fromUint(3)));
        expect(result).to.be.false;
    });

    it('should validate statement', async () => {
        const result = await lastValueFrom(blockService.validateStatementInBlock(leaf, UInt64.fromUint(1)));
        expect(result).to.be.true;
    });

    it('should validate statement - wrong hash', async () => {
        const result = await lastValueFrom(blockService.validateStatementInBlock(leaf, UInt64.fromUint(2)));
        expect(result).to.be.false;
    });

    it('should validate statement - error', async () => {
        const result = await lastValueFrom(blockService.validateStatementInBlock(leaf, UInt64.fromUint(3)));
        expect(result).to.be.false;
    });

    it('should calculate root transaction merkle hash', async () => {
        const result = await lastValueFrom(blockService.calculateTransactionsMerkleRootHash(UInt64.fromUint(1)));
