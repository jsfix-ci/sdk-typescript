...
describe('HashLockHttp', () => {
    ...
    describe('searchHashLock', () => {
        it('should return hash lock page info', async () => {
            ...
            const page = await lastValueFrom(hashLockRepo.search({ address: account.address }));
            const info = page.data[0];
            hash = info.hash;
            expect(page.data.length).to.be.greaterThan(0);

            const infoFromId = await lastValueFrom(hashLockRepo.getHashLock(hash));
            expect(infoFromId).deep.eq(info);
            const merkleInfo = await lastValueFrom(hashLockRepo.getHashLockMerkle(hash));
    ...
    describe('searchHashLock with streamer', () => {
        it('should return hash lock page info', async () => {
            ...
            const info = await lastValueFrom(hashLockRepo.search({ address: account.address, pageSize: 20, order: Order.Asc }));
    ...
    describe('getHashLock', () => {
        it('should return hash lock info given hash', async () => {
            const info = await lastValueFrom(hashLockRepo.getHashLock(hash));
