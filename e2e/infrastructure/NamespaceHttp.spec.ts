...
describe('NamespaceHttp', () => {
    ...
    const validateMerkle = async (namespaceId: NamespaceId): Promise<void> => {
        const merkleInfo = await lastValueFrom(namespaceRepository.getNamespaceMerkle(namespaceId));
    ...
    describe('getNamespace', () => {
        it('should return namespace data given namepsaceId', async () => {
            const namespace = await lastValueFrom(namespaceRepository.getNamespace(defaultNamespaceId));
    ...
    describe('getNamespacesName', () => {
        it('should return namespace name given array of namespaceIds', async () => {
            const namespaceNames = await lastValueFrom(namespaceRepository.getNamespacesNames([defaultNamespaceId]));
    ...
    describe('getLinkedMosaicId', () => {
        it('should return mosaicId given currency namespaceId', async () => {
            const mosaicId = await lastValueFrom(namespaceRepository.getLinkedMosaicId(defaultNamespaceId));
    ...
    describe('getLinkedAddress', () => {
        it('should return address given namespaceId', async () => {
            const address = (await lastValueFrom(namespaceRepository.getLinkedAddress(namespaceId))) as Address;
    ...
    describe('searchNamespace', () => {
        it('should return namespace info', async () => {
            const info = await lastValueFrom(namespaceRepository.search({ ownerAddress: account.address }));
    ...
    describe('searchNamespace with streamer', () => {
        it('should return namespace info', async () => {
            ...
            const info = await lastValueFrom(namespaceRepository.search({ pageSize: 20, order: Order.Desc }));
