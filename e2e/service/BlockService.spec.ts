...
describe('BlockService', () => {
    ...
    describe('Validate transactions', () => {
        it('call block service', async () => {
            ...
            if (transactionInfo && transactionInfo.height !== undefined) {
                const validationResult = await lastValueFrom(blockService.validateTransactionInBlock(transactionHash, transactionInfo.height));
    ...
    describe('Validate receipt', () => {
        it('call block service', async () => {
            const statements = await lastValueFrom(receiptRepository.searchReceipts({ height: UInt64.fromUint(1) }));
            const statement = statements.data[0] as TransactionStatement;
            const validationResult = await lastValueFrom(blockService.validateStatementInBlock(statement.generateHash(), UInt64.fromUint(1)));
            expect(validationResult).to.be.true;
        });
    });
    ...
    describe('Calculate merkler transaction root hash', () => {
        it('Calculate merkler transaction root hash', async () => {
            const calculated = await lastValueFrom(blockService.calculateTransactionsMerkleRootHash(UInt64.fromUint(1)));
            const block = await lastValueFrom(blockRepository.getBlockByHeight(UInt64.fromUint(1)));
